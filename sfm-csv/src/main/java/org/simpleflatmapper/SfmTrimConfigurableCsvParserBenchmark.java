...
public class SfmTrimConfigurableCsvParserBenchmark {
    @Param(value = {"4"})
    ...
    public void parseCsvCallback(Blackhole blackhole, CsvParam csvParam) throws IOException {
        try(Reader reader = csvParam.getReader()) {
            CsvParser.bufferSize(bufferSize * 1024).trimSpaces().reader(reader).read(blackhole::consume);
        }
    }
    ...
    public void parseCsvIterate(Blackhole blackhole, CsvParam csvParam) throws IOException {
        try(Reader reader = csvParam.getReader()) {
            for(String[] row : CsvParser.bufferSize(bufferSize * 1024).trimSpaces().reader(reader)) {
